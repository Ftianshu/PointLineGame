...
namespace Survival.Editor
{
    public sealed partial class DataTableProcessor
    {
        ...
        public DataTableProcessor(string dataTableFileName, Encoding encoding, int nameRow, int typeRow, int? defaultValueRow, int? commentRow, int contentStartRow, int idColumn)
        {
            ...
            {
                GD.PrintErr("Data table file name is invalid.");
            }

            if (!dataTableFileName.EndsWith(".txt", StringComparison.Ordinal))
            {
                GD.PrintErr("Data table file '{0}' is not a txt.", dataTableFileName);
            }

            if (!File.Exists(dataTableFileName))
            {
                GD.PrintErr("Data table file '{0}' is not exist.", dataTableFileName);
            }
            ...
            for (int i = 0; i < lines.Length; i++)
            {
                ...
                {
                    GD.PrintErr("Data table file '{0}', raw Column is '{2}', but line '{1}' column is '{3}'.", dataTableFileName, i, rawColumnCount, rawValue.Length);
                }
            ...
            if (nameRow < 0)
            {
                GD.PrintErr(("Name row '{0}' is invalid.", nameRow));
            }

            if (typeRow < 0)
            {
                GD.PrintErr(("Type row '{0}' is invalid.", typeRow));
            }

            if (contentStartRow < 0)
            {
                GD.PrintErr(("Content start row '{0}' is invalid.", contentStartRow));
            }

            if (idColumn < 0)
            {
                GD.PrintErr(("Id column '{0}' is invalid.", idColumn));
            }

            if (nameRow >= rawRowCount)
            {
                GD.PrintErr(("Name row '{0}' >= raw row count '{1}' is not allow.", nameRow, rawRowCount));
            }

            if (typeRow >= rawRowCount)
            {
                GD.PrintErr(("Type row '{0}' >= raw row count '{1}' is not allow.", typeRow, rawRowCount));
            }

            if (defaultValueRow.HasValue && defaultValueRow.Value >= rawRowCount)
            {
                GD.PrintErr(("Default value row '{0}' >= raw row count '{1}' is not allow.", defaultValueRow.Value, rawRowCount));
            }

            if (commentRow.HasValue && commentRow.Value >= rawRowCount)
            {
                GD.PrintErr(("Comment row '{0}' >= raw row count '{1}' is not allow.", commentRow.Value, rawRowCount));
            }

            if (contentStartRow > rawRowCount)
            {
                GD.PrintErr(("Content start row '{0}' > raw row count '{1}' is not allow.", contentStartRow, rawRowCount));
            }

            if (idColumn >= rawColumnCount)
            {
                GD.PrintErr(("Id column '{0}' >= raw column count '{1}' is not allow.", idColumn, rawColumnCount));
            }
